...
class TalkToChef: Task {
    override val displayName = "Talking To Master Chef"
    override val priority = 0

    override fun canRun(): Boolean {
        return GameState.getSetting(281) == 140
    }

    override fun execute() {
        val npcName = "Master Chef"

        if (!Query.npcs().nameEquals(npcName).isReachable.isAny) {
            Walker.walkTo(Constants.masterChefAreaInside.randomTile)
            return
        }
